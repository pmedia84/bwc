<!-- contact form script from contact page -->
<?php
...
//decode json file and put into an array
...
if (isset($_POST['token'])) {
    ...
    if ($score >= 0.7) { //if the score is above 0.7 then proceed with the form submission. If not then exit the file.
        ...
        $msgtype = filter_var($_POST['msgtype'], FILTER_SANITIZE_SPECIAL_CHARS);
        ...
        if (trim($fname) == '') { //first name
            ...
            exit();
        } else if (trim($msgtype) == '') {
            echo '<div id="response"></div><div class="error_message">Message Subject Not Selected!</div>';
            exit();
        }
        ...
        /////////////////////Admins auto reply/////////////////////////
        //email subject
        $subject = $fname . ' ' . $lname . ' has contacted you from your Contact page regarding ' . $msgtype;
    ...
    <div style="background-color:#3b4142; padding:16px;font-family:sans-serif;">
        ...
        if (!$mail->Send()) {
            ...
            echo "
            <p>Thank you <strong>$fname</strong>, for contacting us regarding $msgtype.<br>We will be in touch with you very soon! </p>"; //if this is all successful then send an auto reply to the client as well
