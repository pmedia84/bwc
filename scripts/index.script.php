...
if($score >=0.7){//if the score is above 0.7 then proceed with the form submission. If not then exit the file.
    ...
    $propertytype = filter_var($_POST['propertytype'], FILTER_SANITIZE_SPECIAL_CHARS);
    ...
    if (trim($fname) == '') { //first name
        ...
        exit();
    } else if (trim($propertytype) == '') {
        echo '<div id="response"></div><div class="error_message">Property Type Not Selected!</div>';
        exit();
    }
/////////////////////////////////////////////////////////Send Emails /////////////////////////////////////////////////////////////////
   ...
   // stored in seperate file
    ...
    ///////////////////Admins auto reply/////////////////////////
        //email subject
        $subject = $fname . ' ' . $lname . ' has requested a quote for their ' . $propertytype . ' property';


        //body of email for website admins
        $body = '<div style="background-color:#3b4142; padding:16px;font-family:sans-serif;">
            <h1 style="text-align:center; color:white;">' . $fname . ' ' . $lname . ' has requested a free quotation from your home page!</h1>
            <div style="background-color: white; padding:16px; border: 10px solid #335a61; border-radius: 10px;">
                ...
                <p style="border-bottom:1px solid;"><strong>Property Type</strong>: ' . $propertytype . ' </p>
            </div>
        </div>';
        ...
        if (!$mail->Send()) {
            ...
            echo "
                <p>Thank you <strong>$fname</strong>, for requesting a quotation from us.<br>We will be in touch with you very soon! </p>"; 

                //if this is all successful then send an auto reply to the client as well
